...
(defn- is-valid-interaction-for-send?
  ...
  (when (and (is-valid-interaction? sender receivers message label active-interaction)
...
(defn- get-send-atomic-interaction
  ...
  [sender receiver message label active-interaction]
  (when (is-valid-interaction-for-send? sender receiver message label active-interaction) active-interaction))

(defn- get-send-recur-identifier-interaction
  "Check the first element in a recursion interaction"
  [monitor sender receiver message label rec]
  (cond
    (satisfies? interactable rec) (get-send-atomic-interaction sender receiver message label rec)
    (satisfies? branchable rec) (get-send-branch-interaction monitor sender receiver message label rec)
    (satisfies? parallelizable rec) (get-send-parallel-interaction monitor sender receiver message label rec)
    (satisfies? closable rec) nil
    (satisfies? identifiable-recur rec) (get-send-recur-identifier-interaction monitor sender receiver message label (get-rec monitor (get-name rec)))
...
(defn- get-send-parallel-interaction
  ...
  [monitor sender receiver message label active-interaction]
  (when-let [_ (first (filter #(cond
                                 (satisfies? interactable %) (get-send-atomic-interaction sender receiver message label %)
                                 (satisfies? branchable %) (get-send-branch-interaction monitor sender receiver message label %)
                                 (satisfies? parallelizable %) (get-send-parallel-interaction monitor sender receiver message label %)
                                 (satisfies? closable %) nil
                                 (satisfies? identifiable-recur %) (get-send-recur-identifier-interaction monitor sender receiver message label (get-rec monitor (get-name %)))
...
(defn- set-send-on-par
  ...
  [sender receivers message label target-interaction monitor]
  (let [pars (let [pars (get-parallel target-interaction)]
               (for [p pars]
                 (cond
                   (satisfies? interactable p)
                   (if (is-valid-interaction-for-send? sender receivers message label p)
                     (assoc-sender-to-interaction p sender)
                     p)
                   (satisfies? branchable p)
                   (let [valid-branch (get-send-branch-interaction monitor sender receivers message label p)]
                     (if (not (nil? valid-branch))
                       (if (satisfies? parallelizable valid-branch)
                         (set-send-on-par sender receivers message label valid-branch monitor)
                         (assoc-sender-to-interaction valid-branch sender))
                       p))
                   (satisfies? parallelizable p)
                   (set-send-on-par sender receivers message label p monitor)
                   (satisfies? identifiable-recur p)
                   (let [valid-rec (get-send-recur-identifier-interaction monitor sender receivers message label (get-rec monitor (get-name p)))]
                     (if (not (nil? valid-rec))
                       (if (satisfies? parallelizable valid-rec)
                         (set-send-on-par sender receivers message label valid-rec monitor)
...
(defn- send-active-interaction-by-parallel
  ...
  [sender receivers message label active-interaction target-interaction monitor]
  (swap! active-interaction
         (fn [inter]
           (if (= (get-id inter) (get-id target-interaction))
             (set-send-on-par sender receivers message label inter monitor)
             (set-send-on-par sender receivers message label target-interaction monitor))))
  true)

(defn- apply-send-to-mon
  "Apply new interaction"
  ([monitor sender receivers message label active-interaction pre-swap-interaction target-interaction]
   (cond
     ...
     (apply-send-to-mon monitor sender receivers message label active-interaction pre-swap-interaction (get-send-branch-interaction monitor sender receivers message label target-interaction))
     (satisfies? parallelizable target-interaction)
     (send-active-interaction-by-parallel sender receivers message label active-interaction target-interaction monitor)
     (satisfies? identifiable-recur target-interaction)
     (apply-send-to-mon monitor sender receivers message label active-interaction pre-swap-interaction (get-rec monitor (get-name target-interaction)))
...
(defn is-valid-send-communication?
  ...
  [monitor sender receivers message label active-interaction]
  (cond
    (satisfies? interactable active-interaction)
    (is-valid-interaction-for-send? sender receivers message label active-interaction)
    (satisfies? branchable active-interaction)
    (first (filter #(is-valid-send-communication? monitor sender receivers message label %) (get-branches active-interaction)))
    (satisfies? parallelizable active-interaction)
    (get-send-parallel-interaction monitor sender receivers message label active-interaction)
    ...
    (if (satisfies? parallelizable active-interaction)
      (when (<= (count (get-parallel active-interaction)) 1)
        (is-valid-send-communication? monitor sender receivers message label (get-rec monitor (get-name active-interaction))))
      (is-valid-send-communication? monitor sender receivers message label (get-rec monitor (get-name active-interaction))))
