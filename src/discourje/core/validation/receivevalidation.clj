...
(defn- get-atomic-interaction
  ...
  (when (is-valid-interaction? sender receiver message label active-interaction) active-interaction))
...
(defn- get-recur-identifier-interaction
  ...
  (cond
    (satisfies? interactable rec) (get-atomic-interaction sender receiver message label rec)
    (satisfies? branchable rec) (get-branch-interaction monitor sender receiver message label rec)
    (satisfies? identifiable-recur rec) (get-recur-identifier-interaction monitor sender receiver message label rec)
    (satisfies? parallelizable rec) (get-parallel-interaction monitor sender receiver message label rec)
...
(defn- get-parallel-interaction
  ...
  (when-let [_ (first (filter
                        #(cond
                           (satisfies? interactable %) (get-atomic-interaction sender receiver message label %)
                           (satisfies? branchable %) (get-branch-interaction monitor sender receiver message label %)
                           (satisfies? parallelizable %) (get-parallel-interaction monitor sender receiver message label %)
                           (satisfies? identifiable-recur %) (get-recur-identifier-interaction monitor sender receiver message label (get-rec monitor (get-name %)))
  ...
  (let [pars (flatten (filter some?
                              (for [par (get-parallel target-interaction)]
                                (let [inter (cond
                                              ...
                                              (remove-from-parallel sender receivers message label par monitor)
                                              (satisfies? interactable par)
                                              (get-atomic-interaction sender receivers message label par)
                                              (satisfies? branchable par)
                                              (get-branch-interaction monitor sender receivers message label par)
                                              (satisfies? identifiable-recur par)
                                              (let [recursion (get-rec monitor (get-name par))
                                                    valid-rec (get-recur-identifier-interaction monitor sender receivers message label recursion)]
                                                (if (nil? valid-rec)
                                                  par
                                                  recursion))
                                              :else
                                  ...
                                  (if (nil? inter)
                                    ...
                                    (cond
                                      ...
                                      (remove-from-parallel sender receivers message label inter monitor)
...
(defn- swap-active-interaction-by-parallel
  ...
  (let [target-parallel-interaction (get-parallel-interaction monitor sender receivers message label target-interaction)]
    (swap! active-interaction
           (fn [inter]
             (let [target (if (= (get-id inter) (get-id target-parallel-interaction))
                            (get-parallel-interaction monitor sender receivers message label inter)
                            target-parallel-interaction)]
               (if (nil? target)
                 inter
                 (remove-from-parallel sender receivers message label target monitor))))))
  true)
...
(defn is-valid-communication?
  ...
  (cond
    ...
    (get-atomic-interaction sender receivers message label active-interaction)
    (satisfies? branchable active-interaction)
    (first (filter #(is-valid-communication? monitor sender receivers message label %) (get-branches active-interaction)))
    (satisfies? parallelizable active-interaction)
    (get-parallel-interaction monitor sender receivers message label active-interaction)
    (satisfies? identifiable-recur active-interaction)
    (is-valid-communication? monitor sender receivers message label (get-rec monitor (get-name active-interaction)))
...
(defn- apply-receive-to-mon
  ...
  [monitor sender receivers message label active-interaction pre-swap-interaction target-interaction]
   (cond
     ...
     (apply-receive-to-mon monitor sender receivers message label active-interaction pre-swap-interaction (get-branch-interaction monitor sender receivers message label target-interaction))
     (satisfies? parallelizable target-interaction)
     (swap-active-interaction-by-parallel sender receivers message label active-interaction target-interaction monitor)
     (satisfies? identifiable-recur target-interaction)
     (apply-receive-to-mon monitor sender receivers message label active-interaction pre-swap-interaction (get-rec monitor (get-name target-interaction)))
