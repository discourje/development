...
!-------------------------------------------------------------------------!
*/
package discourje.examples.tacas2020.npb3.MGThreads;

import discourje.examples.tacas2020.Benchmarks;
import discourje.examples.tacas2020.npb3.MG;
import discourje.examples.tacas2020.npb3.Timer;
...
//c---------------------------------------------------------------------
  ...
  public void setupThreads(MG mg){
    ...
    var m = Benchmarks.newMonitorOrNull(
            "discourje.examples.tacas2020.npb3.MGThreads.spec", "s", num_threads);

    for(int i=0;i<num_threads;i++){
      interp[i]=new Interp(mg,
              Benchmarks.newChan(1, "master", "interp[" + i + "]", m),
              Benchmarks.newChan(1, "interp[" + i + "]", "master", m));
      interp[i].id=i;
      interp[i].start();
      
      psinv[i]=new Psinv(mg,
              Benchmarks.newChan(1, "master", "psinv[" + i + "]", m),
              Benchmarks.newChan(1, "psinv[" + i + "]", "master", m));
      psinv[i].id=i;
      psinv[i].start();

      rprj[i]=new Rprj(mg,
              Benchmarks.newChan(1, "master", "rprj[" + i + "]", m),
              Benchmarks.newChan(1, "rprj[" + i + "]", "master", m));
      rprj[i].id=i;
      rprj[i].start();

      resid[i]=new Resid(mg,
              Benchmarks.newChan(1, "master", "resid[" + i + "]", m),
              Benchmarks.newChan(1, "resid[" + i + "]", "master", m));
      resid[i].id=i;
      resid[i].start();
    } 
  }
