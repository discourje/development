...
!-------------------------------------------------------------------------!
*/
package discourje.examples.tacas2020.npb3.FTThreads;

import discourje.examples.tacas2020.Benchmarks;
import discourje.examples.tacas2020.npb3.FT;
import discourje.examples.tacas2020.npb3.Random;
...
public class FTBase extends Thread{
  ...
  public void setupThreads(FT ft){
    ...
    DiscourjeCoreAsync.Monitor m = Benchmarks.newMonitorOrNull(
            "discourje.examples.tacas2020.npb3.FTThreads.spec", "s", num_threads);

    for(int ii=0;ii<num_threads;ii++){
      doFFT[ii] = new FFTThread(ft,partition1[ii][0],partition1[ii][1],
				   partition2[ii][0],partition2[ii][1],
              Benchmarks.newChan(1, "master", "fft[" + ii + "]", m),
              Benchmarks.newChan(1, "fft[" + ii + "]", "master", m));
      doFFT[ii].id=ii;
      doFFT[ii].start();

      doEvolve[ii] = new EvolveThread(ft,partition2[ii][0],partition2[ii][1],
              Benchmarks.newChan(1, "master", "evolve[" + ii + "]", m),
              Benchmarks.newChan(1, "evolve[" + ii + "]", "master", m));
      doEvolve[ii].id=ii;
      doEvolve[ii].start();
    }
  }
